...
public class Class1
{
    public static void WindowB(int[] collArr, int[] windowArray)
    {
        for (int g = 0; g < collArr[1]; g++)
        {
            while (Deque.Empty() == false && windowArray[g] < windowArray[Deque.Front()])
            {
                Deque.Pop_Front();
            }
        ...
        for (int g = collArr[1]; g < collArr[0]; g++)
        {
            Console.WriteLine(windowArray[Deque.Back()]);

            while (Deque.Empty() == false && Deque.Back() <= g - collArr[1])
            {
                Deque.Pop_Back();
            }
